    ...
    update trips
    	set tourist_id = (select id
						 ...
						 where not exists (select tourist_id 
										   ...
										   OR (i.departure_date = trips.departure_date)
										   OR (i.departure_date = trips.arrival_date)
										   OR (i.arrival_date = trips.departure_date)
										   OR (i.arrival_date = trips.arrival_date))
